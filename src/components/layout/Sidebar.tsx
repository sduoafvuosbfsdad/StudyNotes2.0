    ...
    <div className="flex h-full flex-col">
      <div className="flex h-14 items-center border-b border-border px-3">
        <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
          ...
          {!collapsed ? <span>{t('appName')}</span> : null}
        </div>
      </div>

      <ScrollArea className="h-[calc(100vh-3.5rem)]">
        <nav className="space-y-4 px-2 py-4" aria-label={t('notesNavigation')}>
          {subjects.map((group) => {
            ...
            const translatedSubject = tNote(group.topics[0]).subject;
              ...
              <section key={group.subjectSlug} className="space-y-1">
                ...
                >
                  {!collapsed ? (
                    ...
                    <span title={translatedSubject} className="text-xs font-semibold uppercase tracking-wide">
                      {translatedSubject.charAt(0)}
                    </span>
                  )}
                </button>
                ...
                {isOpen ? (
                  <ul className={cn('space-y-1', collapsed ? 'pl-0' : 'pl-6')}>
                    {group.topics.map((topic) => {
                      const translatedTopic = tNote(topic);
                        ...
                        <li key={`${group.subjectSlug}/${topic.topicSlug}`}>
                          ...
                          >
                            {collapsed ? (
                              ...
                              <span className="truncate">{translatedTopic.title}</span>
                            )}
                          </NavLink>
                        </li>
                      );
    ...
    <>
      ...
      <Sheet open={mobileOpen} onOpenChange={onMobileOpenChange}>
        <SheetContent side="left" className="w-[86vw] max-w-sm p-0 lg:hidden">
          <SheetTitle className="sr-only">{t('notesNavigation')}</SheetTitle>
          <SheetDescription className="sr-only">{t('browseSubjectsDescription')}</SheetDescription>
          <SidebarContent
            collapsed={false}
            onNavigate={() => {
