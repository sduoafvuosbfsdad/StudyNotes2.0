...
If you customize the Dockerfile, keep `NODE_ENV=production` for runtime only (after `npm ci` and `npm run build`) so build tooling from `devDependencies` remains available during image build.
